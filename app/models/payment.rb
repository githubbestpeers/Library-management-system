class Payment < ApplicationRecord
  ...
  Wepay::Application::WEPAY.oauth2_authorize_url(redirect_uri, self.email, self.name)
end
...
def request_wepay_access_token(code, redirect_uri)
  response = Wepay::Application::WEPAY.oauth2_token(code, redirect_uri)
  if response['error']
...
def has_valid_wepay_access_token?
  ...
  response = Wepay::Application::WEPAY.call("/user", self.wepay_access_token)
